...
export function ServiceDetailPage({ serviceId, nav }: { serviceId: string; nav: Nav }) {
  ...
  const isLocalCodexMcp = serviceId === "localCodexMcp";
  const requiresApiKey = !isCustom && !isLocalCodexMcp;
  const canConfigureBaseUrl = isCustom || (!isLocalCodexMcp && Boolean(svc?.baseUrl));
  ...
  return (
    <div className="max-w-xl mx-auto space-y-6">
      ...
      <div className="space-y-5">
        ...
        {/* API Key */}
        {isLocalCodexMcp ? (
          <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/[0.06] px-3 py-2 text-xs leading-5 text-muted-foreground">
            无需 API Key / Token：将直接调用运行 InkOS Studio 后端这台机器上的 <code className="font-mono">codex mcp-server</code>，
            复用本地 Codex CLI 的登录状态。只要终端里本地 Codex 可用，这里就可以测试和保存。
          </div>
        ) : (
          <Field label="API Key">
